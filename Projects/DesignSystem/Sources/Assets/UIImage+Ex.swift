//
//  Assets.swift
//  DesignSystem
//
//  Created by 박서연 on 2024/09/26.
//  Copyright © 2024 simi2-2024. All rights reserved.
//

import UIKit

public extension UIImage {
    // item
    static let icNotifications = DesignSystemAsset.icNotifications.image
    static let icToday = DesignSystemAsset.icToday.image
    static let icHelp = DesignSystemAsset.icHelp.image
    
    // chevron
    static let icChevronLeft = DesignSystemAsset.icChevronLeft.image
    static let icChevronRight = DesignSystemAsset.icChevronRight.image
    
    // arrow
    static let icArrowUp = DesignSystemAsset.icArrowUp.image
    static let icArrowBack = DesignSystemAsset.icArrowBack.image
    static let icArrowDown = DesignSystemAsset.icArrowDown.image
    
    //check
    static let icCheckSmall = DesignSystemAsset.icCheckSmall.image
    static let icCloseSmall = DesignSystemAsset.icCloseSmall.image
    
    static let icEnableCheck = DesignSystemAsset.icEnableCheck.image
    static let icDisableCheck = DesignSystemAsset.icDisableCheck.image
    static let icNoneSelectect = DesignSystemAsset.icNonSelectCheck.image
    
    static let icNoCheck = DesignSystemAsset.icNoCheck.image
    static let icYesCheck = DesignSystemAsset.icYesCheck.image
    
    // login
    static let icKakao = DesignSystemAsset.icKakao.image
    static let icApple = DesignSystemAsset.icApple.image
    
    // tabbar
    static let icMenuO = DesignSystemAsset.icMenuO.image
    static let icMenuX = DesignSystemAsset.icMenuX.image
    static let icNoteO = DesignSystemAsset.icNoteO.image
    static let icNoteX = DesignSystemAsset.icNoteX.image
    static let icActiveMenu = DesignSystemAsset.icActiveMenu.image
    static let icInactiveMenu = DesignSystemAsset.icInactiveMenu.image
    static let icActiveWrite = DesignSystemAsset.icActiveWrite.image
    static let icInactiveWrite = DesignSystemAsset.icInactiveWrite.image
    static let icActiveNoti = DesignSystemAsset.icActiveNoti.image
    
    // character
    static let icHappy = DesignSystemAsset.icHappy.image
    static let icAnger = DesignSystemAsset.icAnger.image
    static let icSad = DesignSystemAsset.icSad.image
    static let icFear = DesignSystemAsset.icFear.image
    static let icOffensive = DesignSystemAsset.icOffensive.image
    static let icSomehow = DesignSystemAsset.icSomehow.image
    static let icGrowingHeart = DesignSystemAsset.icGrowingHeart.image
    static let icEmptyHeart = DesignSystemAsset.icEmptyHeart.image
    static let icNoRecord = DesignSystemAsset.icNoRecord.image
    
    // MARK: - Sample
    static let icSample = DesignSystemAsset.sample.image
}
